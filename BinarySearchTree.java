...
// A method that checks if left is empty if value < focus and checks if right is empty if value > focus
    ...
    Node delete(Node root, int valueDelete)
    {
        ...
        // when we find it we will check from 3 cases to know which to readjust
            ...
            root.value = smallestv(root.rightChild);
            root.rightChild = delete(root.rightChild, root.value);
        }
 
        return root;
    }
    int smallestv(Node focus){
